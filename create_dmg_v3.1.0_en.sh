...
# 复制安装指南和文档
echo "📖 复制安装说明..."
cp INSTALL_GUIDE.md "${MOUNT_DIR}/Documentation/INSTALL_GUIDE.md" 2>/dev/null || echo "  ⚠️ 未找到 INSTALL_GUIDE.md"
cp RELEASE_NOTES_v3.1.0.md "${MOUNT_DIR}/Documentation/RELEASE_NOTES.md" 2>/dev/null || echo "  ⚠️ 未找到 RELEASE_NOTES_v3.1.0.md"
cp UPGRADE_GUIDE_v3.1.0.md "${MOUNT_DIR}/Documentation/UPGRADE_GUIDE.md" 2>/dev/null || echo "  ⚠️ 未找到 UPGRADE_GUIDE_v3.1.0.md"

# 如果有使用说明，也复制
if [ -f "使用说明.md" ]; then
    cp "使用说明.md" "${MOUNT_DIR}/Documentation/"
fi

# 创建快速开始指南（英文文件名，中文内容）
echo "📝 创建快速开始指南..."
cat > "${MOUNT_DIR}/QuickStart.txt" <<EOF
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║             慧眼识鸟 v${VERSION} - 快速开始                   ║
...
🆘 遇到问题？
查看 "Documentation/INSTALL_GUIDE.md" 中的常见问题解答
...
if [ -d "$LIGHTROOM_MODULES_DIR" ]; then
    ln -s "$LIGHTROOM_MODULES_DIR" "${MOUNT_DIR}/Lightroom_Plugins"
else
...
tell application "Finder"
    tell disk "${VOL_NAME}"
        ...
        try
            set position of item "Lightroom_Plugins" of container window to {580, 120}
        end try
...
echo ""
echo "📦 DMG 内容："
echo "  • ${APP_NAME}.app - 主程序（慧眼识鸟）"
echo "  • SuperBirdIDPlugin.lrplugin - Lightroom 插件"
echo "  • Documentation/ - 完整文档"
echo "  • QuickStart.txt - 快速入门指南"
